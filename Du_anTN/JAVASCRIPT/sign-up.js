...
$('#sign-up').on('submit',function() {  /********** Hàm điều kiện dữ liệu nhập **********/
	var error = true;
		...
		if ($('#last-name').val().trim()==''||/[0-9]/.test($('#first-name').val())){
			$('#first-name').prev('span').html('<i class="fa fa-exclamation-circle"></i>'+' Họ, tên của bạn là gì?</br>(không chứa số)');
			error = false;
		} else{
		...
		if ($('#last-name').val().trim()==''||/[0-9]/.test($('#last-name').val())){
			$('#last-name').next('span').html('<i class="fa fa-exclamation-circle"></i>'+' Họ, tên của bạn là gì?</br>(không chứa số)');
			error = false;
		} else{
		...
		if (/^[A-Za-z0-9](([_\.\-]?[a-zA-Z0-9]+)*)@([A-Za-z0-9]+)(([\.\-]?[a-zA-Z0-9]+)*)\.([A-Za-z]{2,})$/.test($('#email').val())){
			...
			$('#email').next('span').html('<i class="fa fa-exclamation-circle"></i>'+' Nhập chính xác email của bạn. ex: abc@gmail.com, </br>abc@yahoo.com');
			error = false;
		}
			/********** Hàm điều kiện pass-word **********/
		if ($('#pass-word').val().trim() == ''|| $('#pass-word').val().length<6 ){
			$('#pass-word').next('span').html('<i class="fa fa-exclamation-circle"></i>'+' Nhập mật khẩu tối thiểu 6 ký tự là số, chữ cái và dấu chấm câu.');
			error = false;
		} else{
