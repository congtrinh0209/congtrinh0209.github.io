          ...
          <div>
            <v-row class="mx-0 my-0 py-0" style="border: 1px solid #D9D9D9; border-top: 0px;">
              ...
              <v-col cols="12" md="3" class="pt-0">
                <span class="label-header">Mã cuộc thi: </span>
                <span class="blue-text font-weight-bold">{{chiTietBaoCao.maBaoCao}}</span>
              </v-col>
              <v-col cols="12" md="3" class="pt-0">
                <span class="label-header">Kỳ - năm cuộc thi: </span>
    ...
    >
      <v-card>
        <v-toolbar
          ...
          class="px-3"
        >
          <v-toolbar-title v-if="readonlyForm && editThanhPhan === null">THÔNG TIN THÀNH PHẦN cuộc thi</v-toolbar-title>
          <v-toolbar-title v-if="editThanhPhan === true && !readonlyForm">CẬP NHẬT THÀNH PHẦN cuộc thi</v-toolbar-title>
          <v-toolbar-title v-if="editThanhPhan === false && !readonlyForm">THÊM THÀNH PHẦN cuộc thi</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn
              icon
        ...
        <v-card-text class="px-2 py-2">
          <dang-ky-thi ref="formThanhPhan" :thongtinthanhphan="thanhPhanEdit" :mauNhap="mauNhapForm" :dataInput="dataInput" :readonly="readonlyForm"></dang-ky-thi>
    ...
    methods: {
      ...
      showAddThanhPhan () {
        ...
        if (vm.danhSachThanhPhan.length && vm.chiTietMauBaoCao.nhapNhieuThanhPhan != 'true') {
          toastr.error('Không thể thêm thành phần. Loại cuộc thi chỉ có 1 thành phần')
          return
        }
      ...
      xoaThanhPhan (item) {
        ...
        let confirm = {
          ...
          message: 'Bạn có chắc chắn muốn xóa thành phần cuộc thi này',
          button: {
            yes: 'Có',
            no: 'Không'
          },
