...
const Component = {
  ...
  methods: {
    handleClickButton({ target }, confirm) {
      if (confirm) {
        ...
        this.$store.commit('SET_CONFIG_CONFIRM_DIALOG', 
          {
            auth: false,
            title: '',
            message: '',
      ...
      this.$store.commit('SET_SHOWCONFIRM', false)
    },
  ...
  computed: {
    isShow () {
      return this.$store.getters.getIsShowConfirm
    },
    dialog () {
      return this.$store.getters.getDialogConfirm
    }
  },
  mounted() {
